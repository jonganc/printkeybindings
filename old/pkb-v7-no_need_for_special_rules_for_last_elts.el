...
(defun list-keymap-symbols
    (keymap &optional (recursive-flag t) (standard-output standard-output))
...
- If CPN-I is a list with a single integer element, say (N), then the (I+1)th
   keymap is at the Nth element of the Ith keymap, (inclusive of the keymap's
   `keymap' symbol, i.e. if we are considering some KEYMAP-I, the next keymap is
   located at (nth N KEYMAP-I) ).
  If CPN-I it is a list with multiple integer elements, say (M N O), the Ith
   keymap has parent keymaps and each element before the last indicates how many
   elements until the next parent. In this example, the first parent is the Mth
   element, the next parent is the Nth element of the previous parent, and the
   next keymap is the Oth element of the second parent (in particular, we could
   simply say that the next keymap is at the (M+N+O)th elemennt of the Ith
   keymap).
  If CPN-K (i.e. the last component) is a list, the keymap is located at the
   element listed.
- If CPN-I is a vector with an integer element [C], the Ith \"keymap\" is
  ...
  If the last component CPN-K is a vector, the keymap is the definition of element C.
- There are special rules for the last element CPN-K in the vector. CPN-K can be:
   - a list with one or more integer elements, say (M N): the target keymap is a
     parent keymap of the Kth keymap, located at the last element listed; in the
     example above, the parent of the Kth keymap is at the Mth element, and the
     target keymap is the parent of this second keymap, at its Nth element
   - an empty list (): the keymap is the definition of the key indicated in the
     previous vectr component
   - a vector [C]: described above.
