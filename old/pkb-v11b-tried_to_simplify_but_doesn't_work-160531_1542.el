...
+ CPN-I is a list:
  ...
  The first element of L can be given by four possibilities:
    - L = (L-1 ...), where L-1 is an (non-negative) integer:
      The next keymap is the L-1 cdr of the element at the cdr pointed to by the
        INT-J's, e.g, if CPN-I is `(M N (P))', the next keymap is at `(nthcdr P
        (nth N (nthcdr M KEYMAP-I)))'.
    - L = ((L-1) ...), where L-1 is an (non-negative) integer:
      The next keymap is the L-1 element of the element at the cdr pointed
  ...
  L either has exactly one element or a second nil element, indicating that the
    location indicated by the INT-J's and L-1 is either the KEYMAP-(I+1) itself
    or a symbol with KEYMAP-(I+1) as its function definition, respectively.
...
sequences."
  ...
  (let (keymaps-found recurse-fcn process-keymap-cons-fcn add-to-keymaps-found-fcn)
    ...
    ;;  If SEARCH-FOR-KEYMAP-IN-SYMBOLS, the first action of `recurse-fcn' is to
    ;;  search for symbols holding KEYMAP in `obarray' is skipped. This is
    ;;  needed when recurse-fcn calls itself recursively but not when initially
    ;;  called.
    (setq recurse-fcn
	  (lambda (keymap cumulative-keymap-location cumulative-key-sequence
			  &optional search-for-keymap-in-symbols)
	    (unless (and search-for-keymap-in-symbols
			 (funcall add-to-keymaps-found-fcn keymap
				  cumulative-keymap-location cumulative-key-sequence))
	      ;; only loop if KEYMAP wasn't found in a symbol  
	      (cl-loop
	       ;;  LOCATION-IN-CUR-CPN is a list that gives the location in the
	       ;;  current component
	       with location-in-cur-cpn = (list)
	       with cur-location
	       with cur-key-sequence
	       with keymap-as-list
	       ;;  skip initial 'KEYMAP symbol
	       for curelt in (cdr keymap)
	       ;;  I indicates which cdr we are on
	       for i = 1 then (cl-incf i)
	       for curcdr = (cdr keymap) then (cdr curcdr)

	       ;; if current cdr is a keymap
	       if (keymapp curcdr)
	       do
	       ;;  we make sure to edit KEYMAP-LOCATION as needed to
	       ;;  indicate if the keymap is a symbol or list
	       (setq cur-location
		     (pkb-append-elt cumulative-keymap-location
				     (pkb-append-elt location-in-cur-cpn
						     i (if (symbolp curcdr)
							   '(nil nil) 
							 '(nil)))))
	       ;;  ADD-TO-KEYMAPS-FOUND-FCN wants the keymap as a list,
	       ;;  not as a symbol.
	       (setq keymap-as-list
		     (if (symbolp curcdr) (symbol-function curcdr) curcdr))
	       (funcall recurse-fcn keymap-as-list
			cur-location cumulative-key-sequence t)
	       return nil
		    ...
		    (setq cur-location
			  (pkb-append-elt cumulative-keymap-location
					  ...
					  (if (symbolp event)
					      (vector event nil)
					    (vector event))))
		    (setq cur-key-sequence
			  (pkb-vconcat-elt cumulative-key-sequence event))
		    ;;  ADD-TO-KEYMAPS-FOUND-FCN wants the keymap as a list, not
		    ;;  as a symbol.
		    (setq keymap-as-list
			  (if (symbolp event) (symbol-function event) event))
		    (unless (funcall add-to-keymaps-found-fcn keymap-as-list
				     cur-location cur-key-sequence)
		      ;;  if KEYMAP is not a symbol, recurse into it
		      (funcall recurse-fcn keymap-as-list cur-location cur-key-sequence))))
		curelt)
	       ;; if current elt not a char-table
	       else
	       (pcase (cdr curelt)
    ...
    ;;  FIND-KEYMAP-SYMBOL-OR-RECURSE-FCN either finds KEYMAP-AS-LIST as a symbol and adds it to KEYMAPS-FOUND, or recurses into keymap
    (setq add-to-keymap-found-or-recurse-fcn
	  (keymap-as-list keymap-location key-sequence))
    ...
    ;;
    ;;  KEYMAP-TO-LOOK-FOR must be the keymap as a list. [we could include in
    ;;  this function the functionality for modifying KEYMAP-LOCATION to
    ;;  indicate that KEYMAP-TO-LOOK-FOR is a symbol. But it's easier to do when
    ;;  we call this function instead]
    (setq
     add-to-keymaps-found-fcn
     (lambda (keymap-to-look-for keymap-location key-sequence)
       (cl-block nil
	 (let (keymap-found symbol-location)
	   ...
	   (when (setq keymap-found (assoc keymaps-found keymap-to-look-for))
	     ...
	     (cl-return t))
	   (when
	       (setq symbol-location (pkb-symbol-location-with-value keymap-to-look-for))
	     (pkb-push-end
	      (list keymap-to-look-for symbol-location
