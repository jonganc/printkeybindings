...
(let* (;; setup
       (1-mods-type-w-binds `((nil bind-1 . bind-opt-1)))

       ;; stuff to translate
       (bks-w-binds-orig `((?1 mk-opt-1 . ,1-mods-type-w-binds)))
       (bks-w-binds (copy-tree bks-w-binds-orig))
       (binds-char-table (make-char-table 'keymap))
       (translate-events-list '((?1 (next 12) t)))
  ...
  (setq bks-w-binds-manual (assq-delete-all ?1 bks-w-binds-manual))
  (add-to-list 'bks-w-binds-manual `(next nil . ,1-mods-type-w-binds) t)
  ;;  manually set binds-char-table-manual
  (setq binds-char-table-manual (make-char-table 'keymap))
  (set-char-table-range binds-char-table-manual 12 1-mods-type-w-binds)
