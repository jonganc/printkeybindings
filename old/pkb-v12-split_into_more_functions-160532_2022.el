...
+ CPN-I is a list:
  ...
  The first element of L can be given by four possibilities:
    ...
    - L = (nil ...):
      The next keymap is exactly the cdr pointed to by the INT-J's of
...
sequences."
  ...
  ;;   always lists.
  (let (keymaps-found recurse-fcn add-to-keymaps-found-or-recurse-fcn
    ...
    (setq recurse-fcn
	  (lambda (cur-keymap cur-keymap-loc cur-key-seq)
	    (cl-loop
	     ;;  LOC-IN-CUR-CPN is a list that gives the location in the current
	     ;;  component
	     with loc-in-cur-cpn = (list)
	     ;;  WORKING-KEYMAP-LOC, WORKING-KEY-SEQ are used
	     with working-keymap-loc
	     with working-key-seq
	     with keymap-as-list
	     for curelt in (cdr cur-keymap)
	     ;;  I indicates which cdr we are on
	     for i = 1 then (cl-incf i)
	     for curcdr = (cdr cur-keymap) then (cdr curcdr)
	     ...
	     if (and (keymapp curcdr)
		     ...
		     (setq working-keymap-loc
			   (pkb-append-elt cur-keymap-loc
					   (pkb-append-elt loc-in-cur-cpn
							   i (if (symbolp curcdr)
								 '(nil nil) 
							       '(nil)))))
		     ...
		     (funcall add-to-keymaps-found-fcn keymap-as-list
			      working-keymap-loc cur-key-seq))
	     return nil
		...
		(when (keymapp defn)
		  (setq working-keymap-loc
			(pkb-append-elt cur-keymap-loc
					...
					(if (symbolp event)
					    (vector event nil)
					  (vector event))))
		  (setq working-key-seq
			(pkb-vconcat-elt cur-key-seq event))
		  ;;  ADD-TO-KEYMAPS-FOUND-FCN wants the keymap as a list, not
		  ;;  as a symbol.
		  (setq keymap-as-list
			(if (symbolp event) (symbol-function event) event))
		  (funcall add-to-keymaps-found-or-recurse-fcn
			   keymap-as-list working-keymap-loc working-key-seq)))
	      curelt)
	     ...
	     ;;  pcase over bindingx
	     (pcase (cdr curelt)
	       ;;  WORKING-KEYMAP-LOC is initially used simply to help mark where the
	       ;;  keymap is
	       ((or
		 ;; binding is a keymap
		 (and (pred keymapp) keymap
		      (pred (lambda (arg) (setq working-keymap-loc 1) t)))
		 ;; binding is (item-name . keymap)
		 `(,(pred stringp) .
		   ,(and (pred keymapp) keymap
			 (pred (lambda (arg) (setq working-keymap-loc 2) t))))
		 ;; binding is (item-name help-string . keymap)
		 `(,(pred stringp) .
		   (,(pred stringp) .
		    ,(and (pred keymapp) keymap
			  (pred (lambda (arg) (setq working-keymap-loc 3) t)))))
		 ;; binding is (menu-item item-name keymap ...)
		 `((,menu-item
		    . (,(pred stringp)
		       . (,(and (pred keymapp) keymap
				(pred (lambda (arg) (setq working-keymap-loc '(3)) t))))))))
		(setq working-keymap-loc
		      (pkb-append-elt cur-keymap-loc
				      (pkb-append-elt loc-in-cur-cpn
						      i 
						      (if (symbolp keymap)
							  (list working-keymap-loc nil)
							(list working-keymap-loc)))))
		(setq keymap-as-list
		      (if (symbolp keymap) (symbol-function keymap) keymap))
		(setq working-key-seq
		      (pkb-vconcat-elt cur-key-seq (car curelt)))
		(funcall add-to-keymaps-found-or-recurse-fcn
			 keymap-as-list working-keymap-loc working-key-seq))))))

    ;;  FIND-KEYMAP-SYMBOL-OR-RECURSE-FCN either finds KEYMAP-AS-LIST as a
    ;;  symbol and adds it to KEYMAPS-FOUND, or recurses into KEYMAP-AS-LIST
    (setq add-to-keymaps-found-or-recurse-fcn
	  (lambda (keymap-as-list keymap-location key-sequence)
	    (unless (or recursive-flag
			(funcall add-to-keymaps-found-fcn keymap-as-list
				 keymap-location key-sequence))
	      ;;  if KEYMAP is not a symbol, recurse into it
	      (funcall recurse-fcn keymap-as-list keymap-location key-sequence))))
    ...
    ;; run `recurse-fcn'
    (funcall recurse-fcn keymap nil [])
