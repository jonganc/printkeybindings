...
"
  (let* ((outp-symb-pref-str
	 ...
	 p-pref-symbs)
    ;; let's define JSTS as a convenience macro for `pkb-join-symbs-to-symb'
    (cl-macrolet
	((jsts (&rest args) `(pkb-join-symbs-to-symb ,@args)))
      ;; let us define a function for defining keys for all keymaps at once
      (cl-flet
	...
	(cl-loop
	 ...
	 (define-key p-pref-symb [?a] (jsts "p-pref" (1+ i) "-a"))))
      ...
      (define-key km3 [?b] 'km3-b)
      (define-key km3 [?\M-b] 'km3-M-b)
      (define-key km3 [?\S-b] 'km3-b)
      (define-key km3 [?\M-\S-b] 'km3-M-S-b)
      (define-key km3 [kp-2] 'km3-kp-2)
      (define-key km3 [M-kp-2] 'km3-M-kp-2)
      
      ;; define a key that is set in parent keymap but is explicitly set to
      ;; nothing in base keymaps
      (define-shadow-12-dfn-3 [?c] 'km3-c)
      
      ;;  define a deeper sequence
      (define-all [?1 ?1 ?1] "1-1-1")
      km1
      ))))
