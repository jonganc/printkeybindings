...
The  variables defined, with each named prefixed OUTPUT-SYMBOL-PREFIX-STRING (e.g. if OUTPUT-SYMBOL-PREFIX-STRING is \"G1000\", the names will be G1000-km1, ...), will be (I is 1,2,3):
	 ...
	 (define-key p-pref-symb (vector (aref i-str 0))
      ...
      (cl-loop for i to 2
		...
		`(easy-menu-define
		   ...
		   ,(concat "km" i-str " An nonoverlapping menu ")
      ...
      (if outp-symb-pref-str
	  (cons km1 outp-symb-pref-str)
	km1
	  )
      ;; (when (not outp-symb-pref-str)
      ;; 	(cl-return-from pkb-make-test-keymap km1))
      ;; (when outp-symb-pref-str
      ;; 	(cl-loop for i to 2
      ;; 		 for i-str = (prin1-to-string (1+ i))
      ;; 		 for km = (nth i kms)
      ;; 		 do
      ;; 		 ;; save km1, km2, ....
      ;; 		 (set (jsts outp-symb-pref-str "km" i-str) km))
      ;; 	(cons km1 outp-symb-pref-str))
      )))
