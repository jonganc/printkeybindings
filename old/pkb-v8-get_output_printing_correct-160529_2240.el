...
the target keymap. It is a vector [CPN-1 CPN-2 ... CPN-K], where each component
...
- If CPN-I is a list with a single integer element, say (N), then the (I+1)th
    keymap is the definition of the Nth element of the Ith keymap.
  If CPN-I is a list with multiple integer elements, say (M N), the Ith keymap
    ...
    Mth cons (beginning from zero), the next parent is the Nth cons of the
    previous parent, and the next keymap is the definition of Oth element of the
    second parent (in particular, we could also say that the next keymap is the
    definition of the (M+N+O)th element of the Ith keymap).
  The last element of the list may instead be an empty list, a list with one
    ...
    - a list with one integer (P) (an empty list is the same as `(0)'), the next
      keymap is the Jth cdr of the event binding; e.g a list (M N (P)) means
      the next keymap is at 
      `(nthcdr P (nth N (nthcdr M KEYMAP-I)))'.
    - a vector with one integer [P], the next keymap is the Jth element of the
      event binding; e.g a list (M N [P]) means the next keymap is at
      `(nth P (nth N (nthcdr M KEYMAP-I)))'.
    Note that there is, essentially, an understood `(1)' at the end of every
    integer list unless otherwise specified, so that (M N) is really (M N (1)),
    since the binding definition is usually cdr 1.
  Note that all location numbers in these lists start at zero and are inclusive
    of the keymap's `keymap' symbol, e.g. CPN-I is `(N)', the next keymap is at
    (cadr (nth N KEYMAP-I)).
- If CPN-I is a vector with an integer element [C], the Ith \"keymap\" is
  ...
  happen and the next element is always simply cdr 1.
...
sequences."
  ...
  (let (keymaps-found recurse-fcn)
    (setq recurse-fcn
	  (lambda (keymap cumulative-keymap-location cumulative-key-sequence)
	    (cl-loop
	     with which-symbols with current-cpn-location = (list)
	     for curelt in (cdr keymap)
	     for i = 1 then (cl-incf i)
	     for curcdr = (cdr keymap) then (cdr curcdr)
	     if (and (eq curelt 'keymap)
		     (setq which-symbols (pkb-which-symbols-with-value curcdr)))
	     do (push (list curcdr (append cumulative-keymap-location (list i [])))
		      keymaps-found)
	     and return nil
	     
	     )	    
	    (cl-loop
	     with which-symbols with current-cpn-location = (vector)
	     for i from 1 to (length keymap)
	     for 
	     for curcdr = (nthcdr i keymap)
	     for curelt = (car curcdr)
	     if (and (eq curelt 'keymap)
		     (setq which-symbols (pkb-which-symbols-with-value curcdr)))
	     do (push (list curcdr (append cumulative-keymap-location )) keymaps-found)
	     and return nil
