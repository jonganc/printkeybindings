...
The LOC'th element of the current keymap is the subsequent keymap."
  ...
  (let (keymaps-found recurse-function find-keymap-symbol-or-recurse-function
		      add-to-keymaps-found-function (binding-in-list (list nil))
		      binding-type
	...
	(lambda (item item-type item-loc)
	  (cl-case item-type
		...
		(setf (car binding-in-list) pcase-binding))
		...
		(setf (car binding-in-list) pcase-binding)))
	     (funcall find-keymap-symbol-or-recurse-function
		      binding-in-list (pkb-vconcat-elt cur-key-seq (car item))
		      cur-keymap-loc binding-type item-loc))
	    (char-table-binding
	     (setf (car binding-in-list) (cdr item))
	     (funcall find-keymap-symbol-or-recurse-function
		      binding-in-list (pkb-vconcat-elt cur-key-seq (car item))
		      cur-keymap-loc item-type item-loc))
	    ((keymap-parent keymap-elt)
	     (setf (car binding-in-list) item)
	     (setf item nil)
	     (funcall find-keymap-symbol-or-recurse-function
		      binding-in-list cur-key-seq cur-keymap-loc item-type item-loc))))
	cur-keymap)))
    ...
    ;;  appropriate.
    ;;
    ;;  BINDING-IN-LIST is the binding that might be a keymap contained as the
    ...
    ;;
    ;;  Note that we input the binding via BINDING-IN-LIST wrapped in a list to
    ...
    (setq
     ...
     (lambda (binding-in-list binding-key-seq containing-keymap-loc item-type item-loc)
       (when (keymapp (car binding-in-list))
	 (when (symbolp (car binding-in-list))
	   (setf binding-symbol-flag t)
	   (setf (car binding-in-list) (symbol-function (car binding-in-list))))
	 ...
	 (when
	     (or (setq keymap-found (assoc keymaps-found (car binding-in-list)))
		 (setq symbol-locations
		       (pkb-symbol-locations-for-value (car binding-in-list)))
		 recursive-flag)
	 ...
	 (cond
	  ...
	  (symbol-locations
	   (push (list (car binding-in-list) symbol-locations
	  ...
	  (recursive-flag
	   (funcall recurse-function (car binding-in-list) containing-keymap-loc
		    binding-key-seq))))))
