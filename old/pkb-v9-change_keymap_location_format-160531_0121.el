...
+ CPN-I is a list:
  If CPN-I is a list with a single integer element, say (N), then the (I+1)th
    keymap is the definition of the Nth element of the Ith keymap. (The
    definition of an element is its cdr, since usually a keymap entry is 
    (EVENT . BINDING) .)
  If CPN-I is a list with multiple integer elements, say (M N O), the Ith keymap
    has parent keymaps and each element in the list before the last indicates
    which cons cell is the next parent. In this example, the first parent is the
    Mth cons (counting from zero), the next parent is the Nth cons of the
    previous parent, and the next keymap is the definition of the Oth element of
    the second parent; in particular, we could also say that the next keymap is
    the definition of the (M + N + O)th element of the Ith keymap.
  Alternatively, the last element of CPN-I may not be an integer, indicating
    that the next keymap is not simply the cdr of an element of KEYMAP-I. There
    are three possibilities: we can have the last element of CPN-I be
    - a list with one non-negative integer (P):
      the next keymap is the Jth cdr of the element in KEYMAP-I pointed to by
      CPN-I; e.g, if CPN-I is `(M N (P))', the next keymap is at
      `(nthcdr P (nth N (nthcdr M KEYMAP-I)))'.
    - a vector with one integer [P]:
      the next keymap is the Jth element of the element in KEYMAP-I pointed to
      by CPN-I; e.g if CPN-I is `(M N [P])', the next keymap is at
      `(nth P (nth N (nthcdr M KEYMAP-I)))'.
    - `(-1)' (i.e. a list containing -1):
      the next keymap is a cdr of KEYMAP-I. e.g., if CPN-I is
      `(M N O (-1))', the next keymap is at
      `(nthcdr O (nthcdr N (nthcdr M KEYMAP-I)))'.
    Note that there is an understood `(1)' at the end of every integer list
      unless either a list or vector is explicitly given, so that a component
      `(M N)' would be really be `(M N (1))'.
    There is one more complication. Sometimes, rather that the keymap being
      attached at the location, it is present in a symbol's function. In this
      case, all of the collections indicating the keymap's placement get a nil
      element appended, i.e.  they become `(P nil)', `[P nil]', `(nil)'
   Note also that all location numbers in these lists start at zero and are
    inclusive of the keymap's `keymap' symbol, e.g. CPN-I is `(N)', the next
    keymap is at `(cdr (nth N KEYMAP-I))'.

+ CPN-I is a vector:
  If CPN-I is a vector with an integer element [C], the Ith \"keymap\" is
    actually a char-table and the next keymap is at the event C in the table;
    note that C may also be a character range (C1 . C2).
  Similar to when CPN-I is a list, a nil element is appended to the vector
    (i.e. it becomes [C nil]) if the definition is a symbol rather than the
    keymap itself.
  In principle, I suppose a vector could have a list element of an integer in a
    list or vector, as for a list component, but I don't think this should ever
    happen and the next element is always simply the definition.
