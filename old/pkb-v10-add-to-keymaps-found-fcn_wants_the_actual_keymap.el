...
(cl-defun pkb-which-symbols-with-value
    (--unique-symbol-123
     &key (test 'eq) stop-if-found-flag ((:obarray obarray-to-search) nil))
...
\(fn VALUE &key TEST STOP-IF-FOUND-FLAG OBARRAY)"
  (let (matches match-fcn)
    ;;  MATCH-FCN: used internally to call if a match is made, so that function
    ;;  quits if STOP-IF-FOUND-FLAG
    (if stop-if-found-flag
	(fset 'match-fcn
	      (lambda (return-value)
		(cl-return-from pkb-which-symbols-with-value (list return-value))))
      (fset 'match-fcn
	    (lambda (add-value) (pkb-push-end add-value matches))))
    (mapatoms
     (lambda (symb)
       ;; since value is bound to VALUE, we skip that variable
       (unless (eq symb 'value)
	(when (funcall test --unique-symbol-123 symb) (match-fcn (cons symb 'symbol)))
       (when (funcall test --unique-symbol-123 (symbol-name symb))
	 (match-fcn (cons  'name symb)))
       (when (and (boundp symb) (funcall test --unique-symbol-123 (symbol-value symb)))
	 (match-fcn (cons symb 'value)))
       (when (and (fboundp symb) (funcall test --unique-symbol-123 (symbol-function symb)))
	 (match-fcn (cons symb 'function)))
       (cl-loop for sublist on (symbol-plist symb) by 'cddr
		for prop = (car sublist)
		when (funcall test --unique-symbol-123 prop)
		do (match-fcn (cons symb 'property)) end
		and when (funcall test --unique-symbol-123 (cadr sublist))
		do (match-fcn (cons (cons symb prop) 'property-value)))))
      obarray-to-search)
    matches))
...
SORT-EVENT-FCN is a function used to sort events before printing them. It takes
...
\(fn KEYMAP &key (STREAM standard-output) (EVENTS-AS-ESCAPE-SEQUENCES-FLAG t) KEY-SEQUENCE-FILTER-FCN PRINT-DEFN-FCN SORT-EVENT-FCN (COPY-ESC-TO-META-FLAG t) UNIQUE-BINDINGS-FLAG)"
      ...
      (setq print-defn-fcn
	    (lambda (event defn indent stream)
		  ...
		  (pkb-print-keymap
		   ...
		   :key-sequence-filter-fcn key-sequence-filter-fcn
		   :sort-event-fcn sort-event-fcn
  ...
  (let (keymap-entries returned-entries add-event-fcn)

    ;;  we define a function ADD-EVENT-FCN for adding events, since it is common
    ...
    (setq
     ...
     (lambda (event defn)
	   ...
	   (and
	    ...
	    (funcall key-sequence-filter-fcn (vconcat prefix-key (vector event))))
	 (push
	  (cons event (if unique-bindings-flag (lookup-key keymap (vector event)) defn))
	  keymap-entries))))

    (map-keymap add-event-fcn keymap)

    ;; include meta events, if desired
    (when copy-esc-to-meta-flag
      (pkb-map-keymap-meta-chars-in-esc-prefix add-event-fcn keymap))
...
(defun pkb-list-keymap-symbols (keymap &optional (recursive-flag t))
  "Find keymaps in KEYMAP that are listed in symbols according to `pkb-which-symbols-with-value'.
If RECURSIVE-FLAG, the function searches in key-sequences that are themselves keymaps. In other words, if not RECURSIVE-FLAG, this function only checks if any parent keymaps of KEYMAP are defined as symbols.

The result is a list with elements (KEYMAP-LOCATION KEY-SEQUENCE SYMBOL-LOCATION). 
KEY-SEQUENCE gives the key-sequence where keymap is defined as a vector. 
SYMBOL-LOCATION indicates where the symbol definition is; it is the output from `pkb-which-symbols-with-value'. 

KEYMAP-LOCATION indicates how to navigate through the elements of KEYMAP to find
the target keymap. It is a list (CPN-1 CPN-2 ... CPN-K), where each component
CPN-I indicate how to find the (I+1)th keymap from the Ith keymap, eventually
leading to the target keymap. Each element CPN-I is either a list or a vector.

+ CPN-I is a list:
  CPN-I will be a list of N integers INT-1, ... with, optionally, a one or two
    element list L at the end, i.e.  `(INT-1 INT-2 ... INT-N L)'; if L
    is not given, it is implied to be `(1)'.
  The integers INT-J just break up the keymap KEYMAP-I into its parent
    keymaps. Thus, the 1st parent keymap is at the INT-1 cdr of KEYMAP-I, the
    parent of the first parent is at the first parent's INT-2 cdr, and
    so. Finally, KEYMAP-(I+1) can be found from the INT-N cdr of the last parent
    keymap using the contents of L. We thus note that KEYMAP-(I+1) is thus found
    by using the contents of L on the (INT-1 + ... + INT-N) cdr of KEYMAP-I.
  The first element L-1 of L can be given by three possibilities:
    - L-1 is an (non-negative) integer:
      The next keymap is the (L-1) cdr of the element at the cdr pointed to by
      the INT-J's, e.g, if CPN-I is `(M N (P))', the next keymap is at 
      `(nthcdr P (nth N (nthcdr M KEYMAP-I)))'.
    - L-1 is a list with an (non-negative) integer (L-1-1):
      The next keymap is the (L-1-1) element of the element at the cdr pointed
      to by the INT-J's; e.g if CPN-I is `(M N ((P)))', the next keymap is at
      `(nth P (nth N (nthcdr M KEYMAP-I)))'.
    - L-1 is nil:
      The next keymap is exactly the cdr pointed to by the INT-J'sof
      KEYMAP-I. e.g., if CPN-I is `(M N O (nil))', the next keymap is at
      `(nthcdr O (nthcdr N (nthcdr M KEYMAP-I)))'.
  L either has one element or a second nil element (e.g. L either is `(L-1)' or
    `(L-1 nil)' ), indicating that the location indicated by the INT-J's and L-1
    is either the KEYMAP-(I+1) itself or a symbol with KEYMAP-(I+1) as its
    function definition, respectively.
   Note also that all location numbers in these lists start at zero and are
    inclusive of the keymap's `keymap' symbol, e.g. if CPN-I is `(N)' or 
    `(N (1))', the next keymap is at `(nthcdr 1 (nth N KEYMAP-I))'.

+ CPN-I is a vector:
  CPN-I is a vector with an integer element [C], the Ith \"keymap\" is
    actually a char-table and the next keymap is at the event C in the table;
    note that C may also be a character range (C1 . C2).
  Similar to when CPN-I is a list, a nil element appended to the vector (i.e. a
    component location [C nil]) means that the definition is a symbol with the
    keymap as its function definition rather than the keymap itself. 
    [In principle, I suppose we could use the same system of including the
    possibility of the keymap being cdr 1, 2,...  of the definition, etc. making
    the second element a list, as above, but I don't think this should ever be
    relevant.]

In the event that a keymap is in multiple locations, KEYMAP-LOCATION,
KEY-SEQUENCE will be lists having corresponding keymap locations and key
sequences."
  ;;  KEYMAPS-FOUND will be a list with elements
  ;;   (KEYMAP-FOUND SYMBOL-LOCATION KEYMAP-LOCATIONS KEY-SEQUENCES)
  ;;  KEYMAP-FOUND is the actual keymap found, SYMBOL-LOCATION is the symbol location.
  ;;  KEYMAP-LOCATIONS, KEY-SEQUENCES are like in the output except they are
  ;;   always lists.
  (let (keymaps-found recurse-fcn process-keymap-cons-fcn add-to-keymaps-found-fcn)

    ;;  RECURSE-FCN is the function at the heart of `pkb-list-keymap-symbols'. It
    ;;   is called recursively (if RECURSIVE-FLAG) to find keymaps.
    (setq recurse-fcn
	  (lambda (keymap cumulative-keymap-location cumulative-key-sequence)
	    (cl-loop
	     ;;  LOCATION-IN-CUR-CPN is a list that gives the location in the
	     ;;  current component
	     with location-in-cur-cpn = (list)
	     for curelt in (cdr keymap)
	     ;;  I indicates which cdr we are on
	     for i = 1 then (cl-incf i)
	     for curcdr = (cdr keymap) then (cdr curcdr)
	     ;; if current cdr is a keymap, is it in `obarray'?
	     if (and (keymapp curcdr)
		     (funcall
		      add-to-keymaps-found-fcn
		      (if (symbolp curcdr) (symbol-function 'curcdr) )
		      (append cumulative-keymap-location
			      (append location-in-cur-cpn (list i (list nil))))
		      cumulative-key-sequence))
	     return nil
	     ;;  current cdr not a keymap in `obarray'
	     ;;  is current element a char-table?
	     else if (char-table-p curelt)
	     (map-char-table
	      (lambda (event defn)
		(when (keymapp))
		)
	      curelt)
	     for 
	     ;;  add keymap to keymaps-founds
	     do (push
		 (list
		  curcdr
		  (append cumulative-keymap-location (append location-in-cur-cpn ()))
    ...
    ;;  PROCESS-KEYMAP-CONS-FCN looks at and processes each cons cell 
    (setq process-keymap-cons-fcn)    

    ;;  ADD-TO-KEYMAPS-FOUND-FCN takes KEYMAP-TO-LOOK-FOR, KEYMAP-LOCATION, and
    ;;  KEY-SEQUENCE and, if appropriate, adds to KEYMAPS-FOUND. Returns t if
    ;;  KEYMAP was found, nil otherwise.
    ;;
    ;;  KEYMAP-TO-LOOK-FOR can be either the keymap as a list or as a symbol
    ;;  with keymap as a function definition. in the latter case,
    ;;  KEYMAP-LOCATION will be appropriately altered.
    (setq
     add-to-keymaps-found-fcn
     (lambda (keymap-to-look-for keymap-location key-sequence)
       (cl-block nil
	 ;;  KEYMAP-TO-LOOK-FOR-AS-LIST is always the keymap definition,
	 ;;  whereas keymap-to-look-for might be a symbol with the keymap as
	 ;;  its function definition
	 (let ((keymap-to-look-for-as-list
		(if (symbolp keymap-to-look-for)
		    (symbol-function keymap-to-look-for)
		  keymap-to-look-for))
	       keymap-found which-symbols)
	   ;; look for keymap in KEYMAP-FOUND and then in `obarray'
	   (unless
	       (setq keymap-found (assoc keymaps-found keymap-to-look-for-as-list))
	     (setq which-symbols
		   (pkb-which-symbols-with-value keymap-to-look-for-as-list)))
	   ;;  if KEYMAP was found and KEYMAP-TO-LOOK-FOR is a symbol, prepare
	   ;;  KEYMAP-LOCATION
	   (when (and (or keymap-found which-symbols)
		      (symbolp keymap-to-look-for))
	     (let ((keymap-location-last-cons
		    (nthcdr (1- (length keymap-location) keymap-location)))
		   ;; (keymap-location-length (length keymap-location))
		   ;; (keymap-location-last-cpn-length (length (last keymap-location)))
		   )
	       (if (vectorp (car keymap-location-last-cons))
		   ;;  last element is a vector
		   (setf (car keymap-location-last-cons)
			 (vconcat (car keymap-location-last-cons) (vector nil)))
		 ;; else, 
		   )
	   ...
	   (when (setq keymap-found (assoc keymaps-found keymap-to-look-for))
	     (pkb-push-end (nth 2 keymap-found) keymap-location)
	     (pkb-push-end (nth 3 keymap-found) key-sequence)
	     (cl-return t))		


	   (cl-symbol-macrolet
	       ((keymap-location-last-cpn
		 (elt (1- keymap-location-length) keymap-location))
		(keymap-location-last-cpn-last-elt
		 (elt (1- keymap-location-length keymap-location
			  (1- (length (last keymap-location)))))))
	     ;; KEYMAP-TO-LOOK-FOR might be a symbol. if so, make it a normal
	     ;; keymap
	     (when (symbolp keymap-to-look-for)
	       (setq keymap-to-look-for (symbol-function keymap-to-look-for))
	       ;; add nil to last element of keymap-location

	       ;; (if (vectorp (elt keymap-location (1- keymap-location-length)))
	       (if (vectorp keymap-location-last-cpn)
		   (cl-callf vconcat keymap-location-last-cpn [nil])
		 (cl-symbol-macrolet
		     ((keymap-location-last-cpn-last-elt
		       (elt (1- keymap-location-last-cpn-length)
			    keymap-location-last-cpn)))
		   
		   )
		 )
	       
	       (setq last-elt-of-last-keymap-location
		     (elt (last keymap-location)
			  (1- (length (last keymap-location)))))
	       (cl-callf (elt (1- key))
		   (if 

		       (listp (last keymap-location))))		
	       )
	     ;; is keymap already in KEYMAPS-FOUND?
	     (when (setq keymap-found (assoc keymaps-found keymap-to-look-for))
	       (pkb-push-end (nth 2 keymap-found) keymap-location)
	       (pkb-push-end (nth 3 keymap-found) key-sequence)
	       (cl-return t))
	     ;; keymap not in KEYMAPS-FOUND. Is it in OBARRAY?
	     (when (setq which-symbols
			 (pkb-which-symbols-with-value keymap-to-look-for))
	       (pkb-push-end keymaps-found
			     (list keymap-to-look-for which-symbols
				   (list keymap-location) (list key-sequence)))
	       (cl-return t))
	     (cl-return nil)))))
