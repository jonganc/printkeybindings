...
;; These package prints the accessible keymaps in a pretty format, i.e. so that bindings are listed as in a computer keyboard and grouped by base-keys. Currently the output is in html format.
...
\(fn (VAR LIST [RESULT]) BODY...)"
  ...
  `(maplist (lambda (,(car spec)) ,@body) ,(cadr spec))
...
(defun pkb-char-table-event-p (event) ;; OK
  ...
  (map-keymap
   (lambda (event defn)
     ...
     (when (and (pkb-include-mk-p event)
       ...
       (if (pkb-char-table-event-p event)
...
original event is deleted."
  ...
  (dolist (translate-event translate-events-list)
    (let* ((event (car translate-event))
      ...
      (when (and (pkb-char-table-event-p event)
	...
	(dolist (target-event (nth 1 translate-event))
	  ...
	  (if asscn
	   ...
	   ;; else BASIC-TYPE does not have an entry in BKS-W-BINDS-SYMB
	    (if (pkb-char-table-event-p target-event)
  ...
  (dolist (one-key-group key-groups)
	...
	(cond
	 ...
	 (t
	  ...
	  (when (pkb-char-table-event-p bk)
