...
kmI-p_q-q-pref-symb - the definition of [?p], [?q ?q] a prefix command
...
kmI-tool-bar - a toolbar."
  (let* ((outp-symb-pref-str
	...
	(cl-loop
	 with p_q-q-pref-symb
	 for i to 2
	 for i-str = (prin1-to-string (1+ i))
	 for p_q-q-pref-symb = (jsts outp-symb-pref-str "km" i-str "-p_q-q-pref-symb" )
	 do
	 ;;  add symbs to P_Q-Q-PREF-SYMBS
	 (pkb-push-end p_q-q-pref-symb p_q-q-pref-symbs)
	 (define-prefix-command p_q-q-pref-symb)

	 (define-key (nth i kms) [?p] p_q-q-pref-symb)
	 (define-key (nth i kms) [?q ?q] p_q-q-pref-symb)

	 ;; define common keys
	 (define-key p_q-q-pref-symb [?a] (jsts "km" i-str "-p_q-q-pref-defn-a"))
	 (define-key p_q-q-pref-symb [?\M-a] (jsts "km" i-str "-p_q-q-pref-defn-M-a"))
	 (define-key p_q-q-pref-symb [?\S-a] (jsts "km" i-str "-p_q-q-pref-defn-S-a"))
	 ;;  non-overlapping keys
	 (define-key p_q-q-pref-symb (vector (aref i-str 0))
	   (jsts "km" i-str (concat "-p_q-q-pref-defn-nonoverlap-" i-str)))
	 for event-mod in '(meta shift)
	 for seq = (vector (event-convert-list (list event-mod (aref i-str 0))))
	 do 
	 (define-key p_q-q-pref-symb seq
	   (jsts "km" i-str "-p_q-q-pref-defn-nonoverlap-" (key-description seq)))))
