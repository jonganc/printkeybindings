...
% The help system is simple.  
Type \kbd{C-h} (or \kbd{F1}) and follow
the directions
%+added
for help info
.  % If you are a first-time user, type \kbd{C-h t} for a
% {\bf tutorial}.
...
\key{apropos: show commands matching a string}{C-h a}
%+added
\key{apropos: symbols}{M-x apropos}
%+added
\metax{apropos: variables}{M-x apropos-variable}

%+added section
Navigating Help:

\key{go back}{C-c C-b}
\key{go forward}{C-c C-f}
\key{follow symbol}{C-c C-c}
...
%+added
\key{move to first indent on line}{M-m}
%+added
\metax{move to top,bottom on screen}{M-0 M-r, M-- M-r}
...
%+added
\key{cycle through mark ring}{C-u C-SPC}
...
%+added
\key{delete white-space before next ')' and move after it}{M-)}
...
\key{set face}{M-o}
%+added
\key{add text to lines}{C-x r t}
%+added
\key{delete text from lines}{C-x r k}
%+added
\key{count lines and characters in region}{M-=}
